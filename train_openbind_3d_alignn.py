# =============================================================================
# MODULE: train_openbind_3d_alignn.py
# PURPOSE: Matched crystallographic ALIGNN training and evaluation pipeline.
...
from torch.utils.data import Subset  # Import selected classes or functions from the named dependency.

from model.ligand_3d_alignn import Ligand3DALIGNN  # Import selected classes or functions from the named dependency.
...
from utils.molecular_features import ATOM_FEATURE_DIM, BOND_FEATURE_DIM  # Import selected classes or functions from the named dependency.
from utils.openbind_ligand_dataset import (  # Import selected classes or functions from the named dependency.
    DEFAULT_DATA_ROOT,  # Perform this step of the surrounding calculation or control-flow block.
    OpenBindGraphDataset,  # Perform this step of the surrounding calculation or control-flow block.
)  # Continue or close the surrounding multiline expression or collection.


PROJECT_ROOT = Path(__file__).resolve().parent  # Bind this name to an intermediate value, configuration setting, or result.
MODEL_LABELS = {"3d_alignn": "Crystallographic 3D ALIGNN"}
...
def make_loaders(dataset, subsets, args):  # Define this callable; its indented block implements the documented operation.
    ...
    loader_type = GraphDataLoader  # Construct or transform graph topology, geometry, or molecular feature data.
...
def build_model(args):  # Define this callable; its indented block implements the documented operation.
    """Construct crystallographic ALIGNN with distance and angle RBFs."""
    return Ligand3DALIGNN(  # Return this computed tensor, metric, object, or collection to the caller.
        atom_feature_dim=ATOM_FEATURE_DIM,  # Construct or transform graph topology, geometry, or molecular feature data.
        bond_feature_dim=BOND_FEATURE_DIM,  # Construct or transform graph topology, geometry, or molecular feature data.
        hidden_dim=args.hidden_dim,  # Bind this name to an intermediate value, configuration setting, or result.
        embedding_dim=args.embedding_dim,  # Create or apply a trainable neural-network component.
        num_layers=args.num_layers,  # Create or apply a trainable neural-network component.
        distance_bins=args.distance_bins,  # Construct or transform graph topology, geometry, or molecular feature data.
        angle_bins=args.angle_bins,  # Construct or transform graph topology, geometry, or molecular feature data.
...
def architecture_report(args, parameter_count):  # Define this callable; its indented block implements the documented operation.
    """Record the exact distance-and-angle representation used by ALIGNN."""
    return {  # Return this computed tensor, metric, object, or collection to the caller.
        ...
        "distance_rbf": True,
        "distance_rbf_bins": args.distance_bins,
        "spatial_cutoff_angstrom": args.spatial_cutoff,
        "maximum_spatial_neighbors": args.max_neighbors,
        "angular_processing": True,
        "angle_rbf_bins": args.angle_bins,
...
def train(args):  # Define this callable; its indented block implements the documented operation.
    ...
    dataset = OpenBindGraphDataset(  # Construct or transform graph topology, geometry, or molecular feature data.
        data_root=data_root,  # Bind this name to an intermediate value, configuration setting, or result.
        mode=args.model,  # Create or apply a trainable neural-network component.
        spatial_cutoff=args.spatial_cutoff,  # Bind this name to an intermediate value, configuration setting, or result.
...
def main() -> None:  # Define this callable; its indented block implements the documented operation.
    """Run the fixed 3D angular graph model; no model selector is required."""
    run_cli(fixed_model="3d_alignn")
