...
def test(args, model, loader, criterion, fabric: Fabric):
    ...
    for g, lg, fg, target, ids in loader:
        ...
        # Get overall error
        test_error = criterion(output, target)
        ...
        indiv_errors.append([error.item() for error in individual_errors])

        # Get predictions and truths as lists
        reals = target.squeeze(dim=0).tolist()
        ...
        for i in range(args.out_dims):
            ...
            test_data[f'truth {args.out_names[i]}'] = reals[i]
        test_data['overall error'] = test_error.item()
        for i in range(args.out_dims):
            test_data[f'error {args.out_names[i]}'] = individual_errors[i].item()
        fabric.log_dict(test_data)
    end_time = time.time()
    fabric.print('-------------------- Testing Finished --------------------')

    fabric.print('Final Results')
    fabric.print(f'Time taken: {end_time - start_time} seconds')
    error = np.mean(test_errors)
    indiv_error = np.mean(indiv_errors, axis=0)
    fabric.print(f'Overall Error: {error}')
    for i in range(len(indiv_error)):
        fabric.print(f'{args.out_names[i]} Error: {indiv_error[i]}')
