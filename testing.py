# =============================================================================
# MODULE: testing.py
# PURPOSE: Original Fabric test entry point reporting structure-level absolute errors.
...
# OUTPUT: Per-structure prediction/error CSV logs.
...
def test(args, model, loader, criterion, fabric: Fabric):  # Define this callable; its indented block implements the documented operation.

    """Evaluate one checkpoint and log per-structure absolute errors."""
    test_errors = []  # Compute or store a loss, error, residual, or regression evaluation statistic.
    indiv_errors = []  # Compute or store a loss, error, residual, or regression evaluation statistic.

    fabric.print('-------------------- Testing Started --------------------')
    ...
    for g, lg, fg, target, ids in loader:  # Iterate over the stated records, layers, batches, or graph elements.
        ...
        # Get overall error
        test_error = criterion(output, target)  # Compute or store a loss, error, residual, or regression evaluation statistic.
        test_errors.append(test_error.item())  # Perform this step of the surrounding calculation or control-flow block.

        # Get per output error
        ...
        indiv_errors.append([error.item() for error in individual_errors])  # Perform this step of the surrounding calculation or control-flow block.

        # Get predictions and truths as lists
        reals = target.squeeze(dim=0).tolist()  # Bind this name to an intermediate value, configuration setting, or result.
        ...
        for i in range(args.out_dims):  # Iterate over the stated records, layers, batches, or graph elements.
            ...
            test_data[f'truth {args.out_names[i]}'] = reals[i]  # Bind this name to an intermediate value, configuration setting, or result.
        test_data['overall error'] = test_error.item()
        for i in range(args.out_dims):  # Iterate over the stated records, layers, batches, or graph elements.
            test_data[f'error {args.out_names[i]}'] = individual_errors[i].item()  # Compute or store a loss, error, residual, or regression evaluation statistic.
    ...
    fabric.print('-------------------- Testing Finished --------------------')

    fabric.print('Final Results')
    fabric.print(f'Time taken: {end_time - start_time} seconds')  # Perform this step of the surrounding calculation or control-flow block.
    error = np.mean(test_errors)  # Compute or store a loss, error, residual, or regression evaluation statistic.
    indiv_error = np.mean(indiv_errors, axis=0)  # Compute or store a loss, error, residual, or regression evaluation statistic.
    fabric.print(f'Overall Error: {error}')  # Perform this step of the surrounding calculation or control-flow block.
    for i in range(len(indiv_error)):  # Iterate over the stated records, layers, batches, or graph elements.
        fabric.print(f'{args.out_names[i]} Error: {indiv_error[i]}')  # Perform this step of the surrounding calculation or control-flow block.
...
def main(args):  # Define this callable; its indented block implements the documented operation.
    ...
    testing_loader = GraphDataLoader(data, collate_fn=data.collate_tt, batch_size=1, shuffle=True)  # Construct or transform graph topology, geometry, or molecular feature data.
